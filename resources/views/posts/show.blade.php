...
<p style="opacity: 0;">
           ...
           <?php
              $some = explode(" ", $posts->created_at);
              $day = explode("-", $some[0])[2];
              $month = explode("-", $some[0])[1];
              if(strlen($posts->content) > 300){
              //$posts[0]->content = substr($posts->content, 0, 300);
              //$posts[0]->content .= ".......";
         ...
         {{$shit =  $posts->image_id}}
            ...
            <div class="row" style="padding-left: 10%;">
                ...
                @endif
                        <a href="{{ url('/news', $posts->id) }}"><h1>{{ $posts->title  }}</h1></a>
                        <p>Posted by {{ $posts->author }}</p>
                        <br><br>
                        <p>  {!! $posts->content !!} </p>
                        <br>
                         @foreach($tags as $tag)
                        @if($tag->post_id == $posts->id)
